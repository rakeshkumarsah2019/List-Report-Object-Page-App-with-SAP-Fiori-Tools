 ...
 */
function transferSettingsAndFlexChanges(configPart, manifest, schemaDefinition, pageKeys, flexChanges, sectionId, sectionTitle) {
    if (!schemaDefinition) {
        return;
    }
    ...
    for (const propertyKey in schemaDefinition['properties'] || schemaDefinition) {
        const exportPropertyRule = common_1.getExportRuleMetadata(configPart, propertyKey);
        if (exportPropertyRule || propertyKey === 'columns') {
            if (propertyKey === 'columns' || (exportPropertyRule && exportPropertyRule.flex)) {
                ...
                if (exportPropertyRule.manifest.import) {
                    configPart[propertyKey] = exportPropertyRule.manifest.import(manifestSection, sectionId);
                }
                else {
...
function fillSections(sectionProperties, manifestSettings, configPart, manifest, jsonSchema, pageKeys, flexChanges) {
    ...
    for (const sectionId in sectionProperties) {
        ...
        if (schemaSection['$ref'] && schemaSection['$ref'].startsWith('#/definitions/ObjectPageSectionV2')) {
            ...
            schemaSection = jsonSchema['definitions']['ObjectPageSectionV2<' + escapedId + '>'];
